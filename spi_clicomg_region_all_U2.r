...
outps <- paste("Usumacinta2.out.ps",sep="")
...
#    curve(dnorm(x, mean(pdf_spi_6083), sd(pdf_spi_6083)), add=TRUE, col="darkblue", lwd=2)
    pyramid.plot(bks11, bks22, top.labels = c("1960-1983","SPI","1984-2007"), laxlab = c(75,50,25,0,25,50,75), raxlab = c(75,50,25,0,25,50,75), lxcol = spi.cols(7), rxcol = spi.cols(7), main = paste(nlat,nlon,sep = ""), labelcex = .5, labels = spilabs, show.values = TRUE, space = 0.2, unit = "")
#    dev.off()
