...
getSPIfromPrec <- function(precipitation){
    ...
    return(spi)
    #>>> return(as.numeric(t(spi.o))) # <- JSS: Más fácil así.
}

# La función que encuentra el SPI para una escala de tiempo es la 
# composición de las dos funciones anteriores:

getSPIfor_k <- getSPIfromPrec %cmp% getPrecOnTimescale  # Esto es: getSPIfor_k(prec, k)
...
test <- function() {
    ...
    spi <- sapply(1:12, function(i) getSPIfromPrec(getPrecOnTimescale(precipitation,i))) # 1:12 son cols a la salida
