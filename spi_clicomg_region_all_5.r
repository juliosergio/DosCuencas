...
getwd()
workdir <- paste("d:/r/spi/",sep="")
setwd(workdir)

ncname <- "CLICOM_pre_1960-2008_mm"  
ncfname <- paste("d:/r/Datos/CLICOMg/",ncname,".nc", sep="")
...
#    curve(dnorm(x, mean(pdf_spi_6083), sd(pdf_spi_6083)), add=TRUE, col="darkblue", lwd=2)
    pyramid.plot(bks11, bks22, top.labels = c("1960-1983","SPI","1984-2007"), laxlab = c(75,50,25,0,25,50,75), raxlab = c(75,50,25,0,25,50,75), lxcol = spi.cols(7), rxcol = spi.cols(7), main = paste(nlat,nlon,sep = ""), labelcex = .5, labels = spilabs)
#    dev.off()
