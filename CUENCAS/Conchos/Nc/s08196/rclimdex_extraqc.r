...
## Extra Quality control routines and TCL/TK button "extra QC " added by Enric Aguilar (C3) and March Prohom (SMC), March 2010
## Ocotber 2011: Fixed bug in flatlines. Code completely modified by Enric Aguilar (C3)
...
allqc = function (master,output,outrange = 3){
tkmessageBox(message = "You are running the Extra QC Functions")
prov<-strsplit(master,"/")
count<-length(prov[[1]])
filename<-prov[[1]][count]
filename<-substr(filename,start=1,stop=(nchar(filename)-4))
...
# the plot will go to series.name_boxplots.pdf
# outliers will be also listed on a file (series.name_outliers.txt)

fourboxes(master,output,save = 1,outrange)
# Will plot a histogram of the decimal point to see rounding problems, for preec, tx, tn
...
cuantatx<<-length(subset(datos$tx,!is.na(datos$tx))) ### This will control if there is no data, added in Guayaquil, 17/01/2011
cuantatn<<-length(subset(datos$tn,!is.na(datos$tn)))
cuantapc<<-length(subset(datos$pc,!is.na(datos$pc)))
if(cuantatx > 0 & cuantatn > 0 ){datos$tr = datos$tx-datos$tn} 
if(cuantapc > 0){prec<-subset(datos,datos$pc>0)}

#Desert filter!

par(mfrow=c(2,2))
# producing boxplots for precip, tmax, tmin, dtr. IQR for default is 3 for temp and 5 for precip
# can be entered as parameter when calling the function. Precip will always be 2 units more than temp

if(cuantapc > 0){respc<-boxplot(prec$pc ~ prec$month,main='NON ZERO PREC',col='blue',range= outrange+2)}
if(cuantatx > 0){restx<-boxplot(datos$tx ~ datos$month,main='TX',col='red',range= outrange)}
if(cuantatn > 0){restn<-boxplot(datos$tn ~ datos$month,main='TN',col='cyan',range= outrange)}
if(cuantatn > 0 & cuantatx > 0){restr<-boxplot(datos$tr ~ datos$month,col='yellow',main= 'DTR',range= outrange)}

# we open a file for writing outliers. First time is not append; rest is append
filena<-paste(output,'_outliers.txt',sep='')
# now we go with precip

if(cuantapc > 0){	
write.table("pc up",file=filena,append=FALSE,quote=FALSE,row.names=FALSE,col.names=FALSE)
## 17/01/2010 : Guayaquil WS: the code would not run with months with no precipitation!

	mismeses<-NULL
	for (meses in 1:12){
		prov<-subset(datos$pc,datos$month == meses & datos$pc > 0)
		if(length(prov)>0){mismeses<-c(mismeses,meses)}
	 }
	baximo=length(mismeses)

for (a in 1:baximo) {		
	prov<-subset(datos,datos$month == mismeses[a] & datos$pc > respc$stats[5,a])
...
if(cuantapc > 0){hist(ispc %% 1,col='blue',main='NON ZERO PREC ROUNDING',breaks=c(seq(-0.1,0.9,0.1)),xlab="")}
if(cuantatx > 0){hist(my$tx %% 1,col='red',main='TX ROUNDING',breaks=c(seq(-0.1,0.9,0.1)),xlab="")}
if(cuantatn > 0){hist(my$tn %% 1,col='cyan',main='TN ROUNDING',breaks=c(seq(-0.1,0.9,0.1)),xlab="")}
...
par(mfrow=c(2,2))
if(cuantapc>0){respc<-boxplot(prec$pc ~ prec$year,main='NON ZERO PREC',col='blue',range=4)}
if(cuantatx>0){restx<-boxplot(datos$tx ~ datos$year,main='TX',col='red',range=3)}
if(cuantatn>0){restn<-boxplot(datos$tn ~ datos$year,main='TN',col='cyan',range=3)}	
if(cuantatx > 0 & cuantatn > 0){restr<-boxplot(datos$tr ~ datos$year,col='yellow',main= 'DTR',range=3)}
if (save == 1) { dev.off() }
...
datos<-read.table(station,col.names=c("year","month","day","pc","tx","tn"), na.strings='-99.9')
tn<-datos$tx # don't worry for the tn/tx here. Just for convenience. Actually, the two functions could be merged
runs<-rle(tn)$lengths
cumruns<-cumsum(runs)
homeruns<-which(runs>3)
hits<-subset(runs,runs>3)
grandslams<-cumruns[homeruns]
datachungo<-datos[grandslams,c(1:3,6)]
datachungo<-cbind(datachungo, hits)
write.table(datachungo,file=filena,quote=FALSE,row.names=FALSE,col.names=FALSE)
rm(list=ls())
}
}
...
getmyfile<-function(){
     ...
     if(length(outdirtmp)<=2){
       outinddir<-paste(strsplit(name,":")[[1]][1],"extraqc",sep=":/")
       ...
       for(i in 2:(length(outdirtmp)-1))
         ...
         outinddir<-paste(outdir,"extraqc",sep="/")

     }

	
       if(!file.exists('extraqc')){ dir.create('extraqc')}

      allqc(master=name,output=outinddir);tkfocus(start1)
...
getPRCP<-function(){
  ...
  mmdd<-NULL
  for(mon in 1:12) mmdd<-c(mmdd,mon*100+c(1:monthday[mon]))
  mmdd.leap<-sort(c(mmdd,229))
  nyrs<-yeare-years+1
  yrs<-years:yeare
  byrs<-endyear-startyear+1
  bss<-c(1:length(yrs))[yrs==startyear]
  bse<-c(1:length(yrs))[yrs==endyear]
  PRCP<-matrix(NA,nyrs,366)
  PRCP.bs<-matrix(NA,nyrs,365)
  for(nth in 1:nyrs){
    yr<-years+nth-1
    if(leapyear(yr)) ydays<-366 else ydays<-365
    PRCP[nth,1:ydays]<-dd[dd[,"year"]==yr,"prcp"]
  }
  for(nth in 1:nyrs){
    yr<-years+nth-1
    if(leapyear(yr)) loc<-c(1:366)[-60] else loc<-c(1:365)
    PRCP.bs[nth,]<-PRCP[nth,loc]
  }
  means<-rep(NA,byrs)
  for(i in 1:byrs) if(sum(is.na(PRCP[bss+i-1,]))<60) means[i]<-mean(PRCP[bss+i-1,],na.rm=T)
# basemean<-sort(means)[length(means)*.1]
  basemean<-min(means,na.rm=T)
  oout<-list()
  oout$PRCP<-PRCP
  oout$PRCP.bs<-PRCP.bs
  oout$means<-means
  oout$basemean<-basemean
  oout$mmdd<-mmdd
  oout$mmdd.leap<-mmdd.leap
  return(oout)
}

find.dry<-function(PRCP){
  obase<-PRCP[1,]
  klen<-10
  for(i in 1:length(obase)) obase[i]<-mean(PRCP[,i],na.rm=T)
  itmp<-c(obase[c((365-klen+1):365,1:365,1:klen)])
  weight<-c(1:klen,(klen+1),klen:1)
  otmp<-obase
  for(i in 1:length(obase)) otmp[i]<-weighted.mean(itmp[i:(i+klen*2)],weight)
  cnt0<-0
  brk<-0
  targ<-order(otmp)[1:30]
  for(i in 1:(365-29)){
    cnt<-sum(i:(i+29)%in%targ)
    if(cnt>cnt0){
      cnt0<-cnt
      brk<-i+14
    }
...
fit.rainy<-function(PRCP,yr,basemean){
  itmp<-PRCP[yr,]
  if(sum(is.na(itmp))<60){
    resi<-itmp-basemean
    A<-rep(0,365)
    for(i in 1:365) A[i]<-sum(resi[1:i],na.rm=T)
    strt<-order(A[1:300],na.last=T,decreasing=F)[1]
    end<-strt+order(A[(strt+1):365],na.last=T,decreasing=T)[1]
  }
  else{ A<-rep(NA,365); strt<-NA; end<-NA}
  if(is.na(strt)==F) if(strt>360) {strt<-NA;end<-NA}
  oout<-list()
  oout$A<-A
  oout$strt<-strt
  oout$end<-end
...
fit2.rainy<-function(PRCP,yr,basemean){
  itmp<-PRCP[yr,]
  strt<-NA
  end<-NA
  ma10<-rep(NA,length(itmp))
  for(i in 1:(length(itmp)-9)) ma10[i+9]<-sum(itmp[i:(i+9)],na.rm=T)
  if(sum(is.na(itmp))<60){
    for(i in 1:(365-49)){
      if(sum(itmp[i:(i+9)],na.rm=T)>=basemean&sum(itmp[(i+10):(i+19)],na.rm=T)>=basemean
         &sum(itmp[(i+20):(i+29)],na.rm=T)>=basemean){
	 strt<-i
	 break
      }
    }
...
run.rainy<-function(){
  library(tkrplot)
  if(!(exists("dd")&exists("nama")&exists("startyear"))) {
    Message<-"Please run Indices calculation first"
    tt2<-tktoplevel()
    tkfocus(tt2)
    tkwm.title(tt2,"Calculating Rainy Season Indices")
    labelText<-tclVar(Message)
    label1<-tklabel(tt2,text=tclvalue(labelText),font=fontHeading1)
    tkgrid(label1)
    OnCancel<-function() {tkdestroy(tt2); tkfocus(start1)}
    OnCancel.but<-tkbutton(tt2,text="   OK   ",command=OnCancel,width=20)
    tkgrid(OnCancel.but)
  }
  else{
    otmp<-getPRCP()
    PRCP<-otmp$PRCP; PRCP.bs<-otmp$PRCP.bs; means<-otmp$means
    yrs<-c(years:yeare); nyrs<-length(yrs); basemean<-otmp$basemean
    mmdd<-otmp$mmdd; mmdd.leap<-otmp$mmdd.leap
    assign("means",means,envir=.GlobalEnv)

    odry<-find.dry(PRCP) # pick dryest day in a year
    obase<-odry$obase; obase.ma<-odry$oma; brk<-odry$brk
    md.brk<-mmdd[brk]
    cmonths<-c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
    cmd.brk<-paste(cmonths[round(mmdd[brk]/100)]," ",mmdd[brk]-round(mmdd[brk]/100)*100,
                   "(",brk,"th day)",sep="")

    if(brk==1) {PRCP.n<-PRCP; mmdd.n<-mmdd; mmdd.leap.n<-mmdd.leap; yrs.n<-yrs}
    else {
      mmdd.n<-matrix(NA,(nyrs-1),366)
      PRCP.n<-matrix(NA,(nyrs-1),366)
      for(i in 1:(nyrs-1)){
        if(leapyear(yrs[i])) {
          otmp1<-PRCP[i,mmdd.leap>=md.brk]
	  omd1<-mmdd.leap[mmdd.leap>=md.brk]
        }
        else{
          otmp1<-PRCP[i,c(1:365)[mmdd>=md.brk]]
	  omd1<-mmdd[mmdd>=md.brk]
        }
        if(leapyear(yrs[i+1])){
          otmp2<-PRCP[(i+1),mmdd.leap<md.brk]
	  omd2<-mmdd.leap[mmdd.leap<md.brk]
        }
        else{
          otmp2<-PRCP[(i+1),c(1:365)[mmdd<md.brk]]
	  omd2<-mmdd[mmdd<md.brk]
        }
        PRCP.n[i,1:(length(otmp1)+length(otmp2))]<-c(otmp1,otmp2)
        mmdd.n[i,1:(length(omd1)+length(omd2))]<-c(yrs[i]*10000+omd1,yrs[i+1]*10000+omd2)
      }
      yrs.n<-yrs[-nyrs]
    }
    assign("PRCP.n",PRCP.n,envir=.GlobalEnv)
    assign("mmdd.n",mmdd.n,envir=.GlobalEnv)

    label.stnname<-tclVar(ofilename)
    Entry.bases<-tclVar(startyear)  # base period start
    Entry.basee<-tclVar(endyear)    # base period end
    Entry.v1mean<-tclVar(round(basemean,1)) # threshold for method 1
    Entry.v2mean<-tclVar("50")   # threshold for method2
    assign("Entry.v1mean",Entry.v1mean,envir=.GlobalEnv)
    assign("Entry.v2mean",Entry.v1mean,envir=.GlobalEnv)

    reRun1<-function(){
      oout1<-NULL
      for(i in 1:dim(PRCP.n)[1]){
        otmp1<-fit.rainy(PRCP.n,i,as.real(tclvalue(Entry.v1mean)))
        if(is.na(otmp1$strt)==F) strt.md<-mmdd.n[i,otmp1$strt] else strt.md<-NA
        if(is.na(otmp1$end)==F) end.md<-mmdd.n[i,otmp1$end] else end.md<-NA
        oout1<-rbind(oout1,c(yrs.n[i],otmp1$strt,otmp1$end,strt.md,end.md))
      }
      colnames(oout1)<-c("year","start_from_break","end_from_break","start_date","end_date")
      assign("oout1",oout1,envir=.GlobalEnv)
      header<-paste("# station:",ofilename," rainy season using method 1 with threshold:",
                    round(as.real(tclvalue(Entry.v1mean)),1),";\n# dry season break at:",
		    brk,"th day, with calender date=",md.brk,sep="")
      ofname<-paste(outinddir,"/",ofilename,"_rainy1.txt",sep="")
      cat(header,file=ofname,fill=20)
      write.table(oout1,file=ofname,append=T,col.names=F,row.names=F,quote=F)
      tkinsert(txt,"0.0",paste("Climatologically Drist Day: ",cmd.brk,
                         "\n Average Rainfall Intensity in Base Period: ",round(mean(means,na.rm=T),1),
			 "\n Lowest Rainfall Intensity in Base Period: ",round(min(means,na.rm=T),1),
			 "\n Thresholds -- Method 1: ", round(as.real(tclvalue(Entry.v1mean)),1),
			 "\n               Method 2: ", round(as.real(tclvalue(Entry.v2mean)),1),
			 "\n\n",sep=""))
    }
    reRun2<-function(){
      oout2<-NULL
      for(i in 1:dim(PRCP.n)[1]){
        otmp2<-fit2.rainy(PRCP.n,i,as.real(tclvalue(Entry.v2mean)))
        if(is.na(otmp2$strt)==F) strt.md<-mmdd.n[i,otmp2$strt] else strt.md<-NA
        if(is.na(otmp2$end)==F) end.md<-mmdd.n[i,otmp2$end] else end.md<-NA
        oout2<-rbind(oout2,c(yrs.n[i],otmp2$strt,otmp2$end,strt.md,end.md))
      }
      colnames(oout2)<-c("year","start_from_break","end_from_break","start_date","end_date")
      assign("oout2",oout2,envir=.GlobalEnv)
      header<-paste("# station:",ofilename," rainy season using method 2 with threshold:",
                    round(as.real(tclvalue(Entry.v2mean)),1),";\n# dry season break at:",
		    brk,"th day, with calender date=",md.brk,sep="")
      ofname<-paste(outinddir,"/",ofilename,"_rainy2.txt",sep="")
      cat(header,file=ofname,fill=20)
      write.table(oout2,file=ofname,append=T,col.names=F,row.names=F,quote=F)
    }

    plotFun1<-function(){
      len1<-oout1[,3]-oout1[,2]; off1<-oout1[,2]
      llen1<-rep(NA,length(len1)*3); ooff1<-llen1
      llen1[c(1:length(len1))*3-2]<-len1; ooff1[c(1:length(len1))*3-2]<-off1
      len2<-oout2[,3]-oout2[,2]; off2<-oout2[,2]
      llen2<-rep(NA,length(len2)*3); ooff2<-llen2
      llen2[c(1:length(len1))*3-1]<-len2; ooff2[c(1:length(len1))*3-1]<-off2
      barplot(height=llen1,offset=ooff1,col="red",ylim=c(0,366),space=0)
      barplot(height=llen2,offset=ooff2,col="blue",ylim=c(0,366),add=T,space=0)
      axis(side=1,at=c(1:length(len1))*3-2,label=oout2[,1])
    }
    plotFun2<-function(){
      xVal<-oout1[,1]
      plot(xVal,rep(-2,length(xVal)),type="n",xlab="",ylab="",ylim=c(0,366))
      for(i in 1:dim(oout1)[1]){
        lines(rep(xVal[i]-.2,2),oout1[i,2:3],col="red",type="l",lwd=2)
        lines(rep(xVal[i]+.2,2),oout2[i,2:3],col="blue",type="l",lwd=2)
      }
    }
    ...
    r.window<-tktoplevel()
    tkfocus(r.window)
    tkwm.title(r.window,"Calculating Rainy Season")
    frameOverall<-tkframe(r.window)
    frameUpper<-tkframe(frameOverall,relief="groove",borderwidth=2)
    frameLower<-tkframe(frameOverall,relief="groove",borderwidth=2)
    frameUpperL<-tkframe(frameUpper,relief="groove",borderwidth=2)
    frameUpperR<-tkframe(frameUpper,relief="groove",borderwidth=2)

    textEntryWidget1<-tkentry(r.window,width="10",textvariable=Entry.v1mean)
    textEntryWidget2<-tkentry(r.window,width="10",textvariable=Entry.v2mean)

    reRunAll.but<-tkbutton(frameUpperL,text="reRun",width=14,command=rreRunAll)
    quit.but<-tkbutton(frameUpperL,text="Quit",width=14,command=onQuit)

    tkgrid(tklabel(frameUpperL,text="           Base period:",width=23),
           tklabel(frameUpperL,text=paste(startyear,"-",endyear,"     ",sep=""),width=14),sticky="w")
    tkgrid(tklabel(frameUpperL,text="Threshold for method 1:"),textEntryWidget1,sticky="w")
    tkgrid(tklabel(frameUpperL,text="Threshold for method 2:"),textEntryWidget2,sticky="w")
    tkgrid(reRunAll.but,quit.but)

    assign("xscr",tkscrollbar(frameUpperR,repeatinterval=1,orient="horizontal",
                  command=function(...)tkxview(txt,...)),envir=.GlobalEnv)
    assign("yscr",tkscrollbar(frameUpperR,repeatinterval=1,
                  command=function(...)tkyview(txt,...)),envir=.GlobalEnv)
    assign("txt",tktext(frameUpperR,bg="white",font="courier",
                  xscrollcommand=function(...)tkset(xscr,...),
                  yscrollcommand=function(...)tkset(yscr,...),
                  width=60,height=6),envir=.GlobalEnv)
    reRun2()
    reRun1()

    tkbind(textEntryWidget1,"<Return>",rreRunAll)
    tkbind(textEntryWidget2,"<Return>",rreRunAll)
    tkgrid(frameUpperR)
    tkgrid(txt,yscr)
    tkgrid(xscr)
#   tkconfigure(txt,state="disabled")
    ...
    tkpack(frameUpperL,frameUpperR,side="left")
    tkpack(frameUpper)
    tkgrid(frameUpper)

    img<-tkrplot(frameLower,fun=plotFun2,hscale=2,vscale=1)
    tkgrid(img)
...
extra.but<-tkbutton(start1,text="Run extra QC",command=getmyfile,width=30)
cal.but<-tkbutton(start1,text="Indices Calculation",command=nastat,width=30)
rainy.but<-tkbutton(start1,text="Rainy Season Indices",command=run.rainy,width=30)
